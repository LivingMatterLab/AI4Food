...
densities = { 
    ...
    'patty': 0.96, # same as ground beef
    'fish': 0.96, # same as ground beef
    'turkey': 0.98,
    'pork': 0.98,
    'chicken': 0.98,
...
def normalize_recipe_by_calories(recipe, target_calories=350):
    """Return a copy of ``recipe`` scaled to the requested calorie total."""
    with open('data/burgers_step8.pkl', 'rb') as f:
        _, ingr_names, calorie_database = pickle.load(f)

    recipe = np.asarray(recipe, dtype=float)
    if recipe.shape != ingr_names.shape:
        raise ValueError(
            f"Recipe has shape {recipe.shape}; expected {ingr_names.shape}."
        )
    if target_calories <= 0:
        raise ValueError("target_calories must be positive.")

    calories_per_gram = np.array(
        [calorie_database[ingr] for ingr in ingr_names], dtype=float
    )
    total_calories = float(np.dot(recipe, calories_per_gram))
    if not np.isfinite(total_calories) or total_calories <= 0:
        raise ValueError("Recipe must have a positive, finite calorie total.")

    return recipe * (target_calories / total_calories)


def plot_recipe(recipe, name=None):
    import matplotlib.pyplot as plt
    with open('data/burgers_step8.pkl', 'rb') as f:
        _, ingr_names, _ = pickle.load(f)
