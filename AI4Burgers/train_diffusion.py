...
def train_diff_mask():
    ...
    for N_epochs in [5000, 10000, 15000]:
        for lr in [1e-5, 1e-4, 1e-3]:
            ...
            with open(f'params/diff_mask_params_epochs_{N_epochs}_lr_{lr}.npy', 'wb') as f:
                pickle.dump(params, f)

            # Sample using the trained params
            trained_score = lambda x, t: score_model.apply(params, x, t)
            rng, step_rng = random.split(rng)
            samples = reverse_sde(step_rng, n_ingr, 50000, drift, dispersion, trained_score).block_until_ready()
            with open(f'params/diff_mask_samples_epochs_{N_epochs}_lr_{lr}.npy', 'wb') as f:
